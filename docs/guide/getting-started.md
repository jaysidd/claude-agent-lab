...
ClawdDesk is a small, hackable multi-agent dashboard built directly on Anthropic's official Claude Agent SDK. It pairs a tiny Express server with a vanilla-JS browser UI so you can chat with several Claude agents, each with its own system prompt, tool allowlist, and model.
...
The whole app is one Node process: Express on port 3333 hands each chat to the SDK's `query()` call and renders the async stream of events back to the browser. There is no build step (the frontend is plain HTML, CSS, and JS) and no separate renderer. The SDK does the heavy lifting; ClawdDesk is the thin, readable layer on top.
