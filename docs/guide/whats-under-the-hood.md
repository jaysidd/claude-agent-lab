...
ClawdDesk is not a wrapper around Claude. The Claude Agent SDK *is* the engine, and this app is the thinnest possible UI on top of it.
...
No. It uses the Claude Agent SDK, which is Claude Code's loop published as a library. ClawdDesk just renders that loop in a browser.
...
No. ClawdDesk is Claude-only by design. If you need OpenAI, Ollama, or local models, that is a different tool.
